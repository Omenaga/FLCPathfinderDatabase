// Load earned honors only when this separate dialog is opened.
...
export default function HonorsOverlay({ id, onClose }: { id: number; onClose: () => void }) {
  const [records, setRecords] = useState<
    { id: number; year_earned: string | null; honors: { name: string } }[] | null
  >(null)
  ...
  useEffect(() => {
    ...
    getSupabase()
      .from('honors_earned')
      .select('id,year_earned,honors(name)')
      .eq('pathfinder_id', id)
      .order('year_earned', { ascending: false, nullsFirst: false })
      ...
      {error ? (
        ...
        <HistoryRecords
          ...
          records={records.map((record) => ({
            year: record.year_earned,
            detail: record.honors.name,
          }))}
        />
      )}
    </Modal>
