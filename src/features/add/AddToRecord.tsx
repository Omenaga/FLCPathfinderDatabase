...
import { LEVELS, DRUMS, OPERATIONS, EVENTS, PLACEMENTS, PBE_REGIONS, period, type Pathfinder } from '../../lib/pathfinders'
...
export default function AddToRecord({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!controller.signal.aborted) { setCatalog({ staff: staff.data.map(row => row.title), books: books.data }); setCurrentYear(Number(season.data.slice(0,4))); setLoadError('') }
  ...
  const entry: Json = kind === 'level' ? { kind, name, outcome } : kind === 'staff' ? { kind, details: [name] }
    ...
    : kind === 'honor' ? { kind, honor_id: honor?.id ?? null } : { kind, details: selectedDetails, ...(kind === 'pbe' && Object.keys(pbeResults).length ? { results: pbeResults } : {}) }
  ...
  return <Modal title={title} header={<h2>{title}</h2>} onClose={onClose} closeDisabled={step === 'saving'} wide={step === 'people'}>
    {step === 'saving' ? <p role="status">Saving information to the selected profiles…</p> : step === 'error' ? <>
      ...
      {(['added','already','error'] as const).filter(status => results.some(row => row.status === status)).map(status => <section key={status}><h3>{status === 'added' ? 'Added' : status === 'already' ? 'Already had this information' : 'Not added'} ({results.filter(row => row.status === status).length})</h3>
        <ul>{results.filter(row => row.status === status).map(row => <li key={row.id}><strong>{row.name}</strong>{row.reason && <p className={status === 'error' ? 'error' : 'muted'}>{row.reason}</p>}{row.year_added && status === 'already' && <span> — Years Active updated</span>}</li>)}</ul></section>)}
      ...
      <form onSubmit={next} className="record-form"><div className="record-fields">
        ...
        {kind === 'pbe' && <div className="pbe-summary">
          <MultiSelect label="PBE regions (optional)" values={PBE_REGIONS.filter(region => region in pbeResults).map(region => `${region} / ${pbeResults[region]}`)}
            options={PBE_REGIONS.flatMap(region => PLACEMENTS.map(placement => `${region} / ${placement}`))}
            exclusiveKey={option => option.split(' / ')[0]} variantLabel={option => option.split(' / ')[1]}
            isOptionDisabled={option => PBE_REGIONS.slice(0, PBE_REGIONS.indexOf(option.split(' / ')[0] as typeof PBE_REGIONS[number])).some(region => !pbeResults[region])}
            onChange={values => {
              const next = Object.fromEntries(values.map(value => value.split(' / ')))
              const firstMissing = PBE_REGIONS.findIndex(region => !next[region])
              setPbeResults(Object.fromEntries(PBE_REGIONS.slice(0, firstMissing < 0 ? PBE_REGIONS.length : firstMissing).map(region => [region, next[region]])))
            }} />
          <p className="muted">Choose placements in order: Area, State, Union, Divisional. Removing a region also removes later regions.</p>
        </div>}
