// Entry points for creating a person or adding history. New profiles require a second click within five seconds.
...
export default function AddRecords({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <div className="record-actions" role="group" aria-label="Add records">
...
function AddRecordModal({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  // Reuse the request ID on retry so a delayed response cannot create a second person.
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (deadline !== null && Date.now() < deadline) {
      ...
      const details = {
        firstName: String(fields.get('first_name') ?? '').trim(),
        lastName: String(fields.get('last_name') ?? '').trim(),
        birthday: String(fields.get('birth_date') ?? ''),
      }
      setDeadline(null)
      setSaveError('')
      setSaving(true)
      ...
      try {
        const { data, error } = await getSupabase().rpc('add_member_record', {
          ...
          p_current_title:
            (status === 'Pathfinder' || status === 'Staff') && titles.length ? titles : null,
          p_school_year: year,
        })
        if (error) throw error
        if (!Number.isInteger(data))
          throw new Error('Could not confirm the save. Retry with the same details.')
        setSummary(details)
        setSaved(true)
        onAdded()
      } catch (error) {
    ...
    return (
      <Modal title="Adding Record" header={<h2>Adding Record</h2>} onClose={onClose} closeDisabled>
        <div className="record-feedback" role="status" aria-live="polite" aria-busy="true">
          <p>Saving the new profile…</p>
          <p className="muted">Please wait while the record is submitted.</p>
        </div>
      </Modal>
    )
  if (saveError)
    return (
      <Modal title="Unable to Add Record" header={<h2>Unable to Add Record</h2>} onClose={onClose}>
  ...
  if (saved && summary)
    return (
      <Modal title="Record Added" header={<h2>Record Added</h2>} onClose={onClose}>
        ...
        <section className="profile-summary" aria-label="New profile summary">
          ...
          <dl className="profile-records">
            <div>
              <dt>First Name</dt>
              <dd>{summary.firstName}</dd>
            </div>
            <div>
              <dt>Last Name</dt>
              <dd>{summary.lastName || 'Not recorded'}</dd>
            </div>
            <div>
              <dt>Status</dt>
              <dd>{status}</dd>
            ...
            <div>
              ...
              <dd>
                {summary.birthday
                  ? `${summary.birthday.slice(5, 7)}/${summary.birthday.slice(8, 10)}/${summary.birthday.slice(0, 4)}`
                  : 'Not recorded'}
              </dd>
            </div>
            <div>
              <dt>Class/Title</dt>
              <dd>{hasTitle ? titles.join(', ') || 'Not recorded' : 'N/A'}</dd>
            </div>
            <div>
              <dt>Current Year</dt>
              <dd>{displayYear}</dd>
        ...
        <fieldset disabled={saving} className="record-fieldset">
          <div className="record-fields">
            ...
            <label>
              ...
              />
              <small className="muted">Leave blank if unknown.</small>
            </label>
            <label>
              Status
              <select
            ...
            <label>
              ...
              <input
                ...
                autoComplete="bday"
              />
              <small className="muted">Optional if unknown.</small>
            </label>
            <label>
              Current Year
              <input
                readOnly
                value={year ? `${year.slice(0, 4)}-${Number(year.slice(0, 4)) + 1}` : ''}
                placeholder={loading ? 'Loading current year…' : 'Unavailable'}
              />
              <small className="muted">Current club school year.</small>
            </label>
            <div className={status === 'Staff' ? 'staff-title-select' : undefined}>
              {status === 'Pathfinder' ? (
                <label>
                  Class/Title
                  <select
                    value={titles[0] ?? ''}
                    onChange={(e) => setTitles(e.target.value ? [e.target.value] : [])}
                  >
              ...
              ) : hasTitle && !loading && !error ? (
                <MultiSelect
                  key={status}
                  label="Class/Title"
                  values={titles}
                  options={orderedStaffTitles}
                  optionGroup={status === 'Staff' ? staffTitleGroup : undefined}
                  variantLabel={status === 'Staff' ? staffTitleLabel : undefined}
              ...
              {hasTitle && (
                <small className="muted">
                  {status === 'Pathfinder'
                    ? 'Choose one class, or leave blank if unknown.'
                    : 'Choose one or more, or leave blank if unknown.'}
                </small>
              )}
            </div>
          </div>
          ...
          >
            <p role="status">
              {saving
                ? 'Saving record…'
                : deadline !== null
                  ? 'Ready to add this profile? Click Confirm before the timer expires.'
                  : 'Review the details, then click Add to confirm.'}
            </p>
            {saveError && (
            ...
            <div className="actions">
              <button type="submit" disabled={loading || !!error || saving}>
                {saving ? 'Saving…' : deadline !== null ? `Confirm (${seconds}s)` : 'Add'}
              </button>
