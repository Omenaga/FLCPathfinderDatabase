// Exercise early migration stages in an isolated PostgreSQL engine, including validation and access rules.
...
before(async () => {
  ...
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260908000000_pathfinder_database.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    `insert into private.staff_access values ('${viewer}', 'viewer', now()), ('${editor}', 'editor', now());`,
  )
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911000000_auth_users_are_staff.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911010000_current_data.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911020000_registration_status.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911030000_searchable_status.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(`
  ...
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911040000_pbe_tlt_arrays.sql', import.meta.url),
      'utf8',
    ),
  )
  ...
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911050000_pbe_year_history.sql', import.meta.url),
      'utf8',
    ),
  )
  const tltMigration = await readFile(
    new URL('../supabase/migrations/20260911060000_tlt_year_history.sql', import.meta.url),
    'utf8',
  )
  ...
  assert.deepEqual((await db.query('select operations from tlt')).rows[0].operations, [
    'Records',
    'Teaching',
  ])
  ...
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260911070000_activity_year_search.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260912000000_event_year_search.sql', import.meta.url),
      'utf8',
    ),
  )
  await db.exec(
    await readFile(
      new URL(
        '../supabase/migrations/20260912010000_detail_participation_search.sql',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  await db.exec(
    await readFile(
      new URL(
        '../supabase/migrations/20260912020000_remove_core_participation.sql',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  await db.exec(
    await readFile(
      new URL('../supabase/migrations/20260912030000_history_detail_filters.sql', import.meta.url),
      'utf8',
    ),
  )
})
...
test('validates level objects, unique options and consecutive school years', async () => {
  for (const expression of [
    ...
    `years_active = '["2024-2025", "2024-2025"]'`,
    `years_active = '{}'`,
    `levels = '[{"name":"Friend"}]'`,
    `levels = '[{"name":"Friend","advanced":null}]'`,
    `levels = '[{"name":"Other","advanced":false}]'`,
...
test('editor creates a member and paired activity and Red Zone histories', async () => {
  await asRole('authenticated', editor, async () => {
    assert.equal(
      (await db.query('select public.current_staff_role() as role')).rows[0].role,
      'editor',
    )
    ...
    const id = (await db.query("select id from pathfinders where name='Synthetic Member'")).rows[0]
      .id
    await db.exec(`
    ...
    assert.deepEqual(
      (await db.query('select year,placement from red_zone_archery order by year')).rows,
      [
        { year: 2024, placement: '1st Place' },
        { year: 2025, placement: '2nd Place' },
      ],
    )
    await reject(
      `insert into drum_corps(pathfinder_id,years,drum_played) values (${id}, '["2024-2025"]', 'Snare')`,
      '23505',
    )
    await reject(
      `insert into red_zone_archery(pathfinder_id,year,placement) values (${id},2026,'Winner')`,
    )
    await reject(
      `insert into red_zone_archery(pathfinder_id,year,placement) values (${id},2024,'3rd Place')`,
      '23505',
    )
  ...
  for (const uid of [viewer, outsider]) {
    await asRole('authenticated', uid, async () => {
      ...
      for (const table of tables) {
        ...
        assert.ok(
          (await db.query(`update ${table} set ${column}=${column} returning ${column}`)).rows
            .length > 0,
        )
...
test('current data preserves historical search, validates fields and enforces access', async () => {
  await asRole('authenticated', editor, async () => {
    const id = (await db.query("select id from pathfinders where name='Synthetic Member'")).rows[0]
      .id
    await db.query(
      `insert into current_data(pathfinder_id,school_year,status,grade,class_level,current_activities)
      values ($1,'2026-2027','returning',7,'Explorer','["Drums"]')`,
      [id],
    )
    ...
    assert.equal(
      (
        await db.query(
          `select * from member_search where search_years @> '["2024-2025"]' and levels @> '[{"name":"Friend","advanced":true}]' `,
        )
      ).rows.length,
      1,
    )
    await reject(
      `insert into current_data(pathfinder_id,school_year) values (${id},'2026-2027')`,
      '23505',
    )
    ...
    await reject(
      `update current_data set current_activities='["Unknown"]' where pathfinder_id=${id}`,
    )
    ...
    assert.equal(
      (await db.query('select has_current_data from member_search where id=$1', [id])).rows[0]
        .has_current_data,
      false,
    )
    assert.equal(
      (await db.query('select status from member_search where id=$1', [id])).rows[0].status,
      'graduated',
    )
    assert.ok(
      (await db.query('select * from member_search where not has_current_data')).rows.length > 0,
    )
...
test('effective status filters distinguish registration and graduation', async () => {
  await asRole('authenticated', editor, async () => {
    for (const status of ['new', 'returning']) {
      const id = (
        await db.query('insert into pathfinders(name) values ($1) returning id', [
          'Status ' + status,
        ])
      ).rows[0].id
      await db.query(
        'insert into current_data(pathfinder_id, school_year, status) values ($1, $2, $3)',
        [id, '2026-2027', status],
      )
      assert.equal(
        (await db.query('select name from member_search where status=$1', [status])).rows[0].name,
        'Status ' + status,
      )
    }
    const inactive = (await db.query("select * from member_search where status='unregistered'"))
      .rows
    assert.ok(inactive.length > 0)
    assert.ok(inactive.every((row) => !row.has_current_data))
    assert.equal(
      (await db.query("select name from member_search where status='graduated'")).rows[0].name,
      'Synthetic Member',
    )
  })
})
...
test('PBE history preserves years and books in one row and validates each pair', async () => {
  const id = (await db.query("select id from pathfinders where name='Migration fixture'")).rows[0]
    .id
  const rows = (await db.query('select history from pbe where pathfinder_id=$1', [id])).rows
  assert.deepEqual(rows, [
    {
      history: [
        { year: '2013-2014', books: [] },
        { year: '2014-2015', books: [] },
  ...
  await asRole('authenticated', editor, async () => {
    const history = [
      ...
      { year: '2021-2022', books: ['1 Kings', 'Ruth'] },
      { year: '2010-2011', books: [] },
    ]
    await db.query('update pbe set history=$1 where pathfinder_id=$2', [
      JSON.stringify(history),
      id,
    ])
    assert.deepEqual(
      (await db.query('select history from pbe where pathfinder_id=$1', [id])).rows[0].history,
      history,
    )
    for (const invalid of [
      ...
      [{ year: '2013-2014' }],
      [{ year: null, books: [] }],
      [{ year: '2013-2014', books: null }],
      [{ year: '2013-2014', books: [1] }],
      [{ year: '2013-2014', books: [], extra: true }],
    ]) {
      await assert.rejects(
        db.query('update pbe set history=$1 where pathfinder_id=$2', [JSON.stringify(invalid), id]),
        (error) => error.code === '23514',
      )
    }
    await reject(
      `insert into pbe(pathfinder_id,history) values (${id},'[{"year":"2022-2023","books":["John"]}]')`,
      '23505',
    )
  ...
  await reject(
    "delete from pbe_year_books where school_year='2021-2022' and book_name='Ruth'",
    '23503',
  )
  await reject(
    "update pbe_year_books set book_name='Other' where school_year='2014-2015' and book_name='Matthew'",
    '23503',
  )
...
test('TLT calendar history accepts all valid operations per year and enforces year bounds', async () => {
  const id = (await db.query("select id from pathfinders where name='Migration fixture'")).rows[0]
    .id
  const currentYear = (await db.query('select extract(year from current_date)::integer as year'))
    .rows[0].year
  const operations = ['Administrative', 'Outreach', 'Teaching', 'Activity', 'Records', 'Counseling']
  const history = [
    { year: 2017, operations },
    { year: currentYear, operations },
  ]
  await asRole('authenticated', editor, async () => {
    await db.query('insert into tlt(pathfinder_id,history) values ($1,$2)', [
      id,
      JSON.stringify(history),
    ])
    assert.deepEqual(
      (await db.query('select history from tlt where pathfinder_id=$1', [id])).rows[0].history,
      history,
    )
    await reject(
      `insert into tlt(pathfinder_id,history) values (${id},'[{"year":2018,"operations":[]}]')`,
      '23505',
    )
    for (const invalid of [
      null,
      ...
      [null],
      [2017],
      [{ year: 2016, operations: [] }],
      [{ year: currentYear + 1, operations: [] }],
      [{ year: '2017', operations: [] }],
      [{ year: '2017-2018', operations: [] }],
      [{ year: 2017.5, operations: [] }],
      [{ year: 1e20, operations: [] }],
      [{ year: 2017, operations: ['Other'] }],
      [{ year: 2017, operations: ['Teaching', 'Teaching'] }],
      [{ year: 2017, operations: ['Teaching, Records'] }],
      [{ year: 2017, operations: [1] }],
      [
        { year: 2017, operations: [] },
        { year: 2017, operations: ['Records'] },
      ],
      [{ year: 2017 }],
      [{ year: null, operations: [] }],
      [{ year: 2017, operations: null }],
      [{ year: 2017, operations: [], extra: true }],
    ]) {
      await assert.rejects(
        db.query('update tlt set history=$1 where pathfinder_id=$2', [JSON.stringify(invalid), id]),
        (error) => error.code === '23514',
      )
    }
    await db.query('update tlt set history=$1 where pathfinder_id=$2', [
      JSON.stringify([{ year: 2017, operations: [] }]),
      id,
    ])
  })
})
...
test('activity/year search matches linked histories and requires every pair', async () => {
  await asRole('authenticated', editor, async () => {
    const find = async (pairs) =>
      (
        await db.query('select name from member_search where search_activity_years @> $1::jsonb', [
          JSON.stringify(pairs),
        ])
      ).rows
    assert.deepEqual(await find(['Drill (2024)', 'Drums (2026)', 'PBE (2025)', 'TLT (2025)']), [
    ...
    const id = (
      await db.query(
        "insert into pathfinders(name) values ('Current activity fixture') returning id",
      )
    ).rows[0].id
    await db.query(
      'insert into current_data(pathfinder_id,school_year,status,current_activities) values ($1,public.current_club_year(),\'new\',\'["Drill","TLT"]\')',
      [id],
    )
...
test('Red Zone search requires exact event/year pairs across every event table', async () => {
  await asRole('authenticated', editor, async () => {
    const find = async (pairs) =>
      (
        await db.query('select name from member_search where search_event_years @> $1::jsonb', [
          JSON.stringify(pairs),
        ])
      ).rows
    for (const name of names) {
      assert.deepEqual(await find([`${name} (2024)`, `${name} (2025)`]), [
        { name: 'Synthetic Member' },
      ])
      assert.deepEqual(await find([`${name} (2026)`]), [])
    }
    assert.deepEqual(await find(['Archery (2024)', 'Knots Relay (2025)']), [
      { name: 'Synthetic Member' },
    ])
  ...
  await asRole('authenticated', null, async () =>
    assert.equal((await db.query('select search_event_years from member_search')).rows.length, 0),
  )
})
...
test('detail records alone drive both dated and any-year participation filters', async () => {
  await asRole('authenticated', editor, async () => {
    const id = (
      await db.query("insert into pathfinders(name) values ('Detail-only member') returning id")
    ).rows[0].id
    await db.query('insert into drill values ($1,\'["2013-2014"]\')', [id])
    await db.query(
      "insert into drum_corps(pathfinder_id,years,drum_played) values ($1,'[\"2014-2015\"]','Snare')",
      [id],
    )
    await db.query(
      'insert into pbe(pathfinder_id,history) values ($1,\'[{"year":"2014-2015","books":["Matthew"]}]\')',
      [id],
    )
    await db.query(
      'insert into tlt(pathfinder_id,history) values ($1,\'[{"year":2017,"operations":["Teaching"]}]\')',
      [id],
    )
    for (const t of events) {
      const named = ['honor_evaluations', 'bible_events'].includes(t)
      await db.query(
        `insert into red_zone_${t}(pathfinder_id,year,placement${named ? ',name' : ''}) values ($1,2014,'Participation'${named ? ",'Example'" : ''})`,
        [id],
      )
    }
    const row = (await db.query('select * from member_search where id=$1', [id])).rows[0]
    ...
    const legacy = (
      await db.query("insert into pathfinders(name) values ('Member without details') returning id")
    ).rows[0].id
    const empty = (
      await db.query('select search_activities,search_events from member_search where id=$1', [
        legacy,
      ])
    ).rows[0]
...
test('instrument, operation and placement filters stay paired with their actual year', async () => {
  await asRole('authenticated', editor, async () => {
    const find = async (column, filters) =>
      (
        await db.query(
          `select name from member_search where name='Synthetic Member' and ${column} @> $1::jsonb`,
          [JSON.stringify(filters)],
        )
      ).rows
    assert.equal(
      (await find('search_activity_details', [{ name: 'Drums', year: 2024, detail: 'Snare' }]))
        .length,
      1,
    )
    assert.equal(
      (await find('search_activity_details', [{ name: 'Drums', year: 2024, detail: 'Bass' }]))
        .length,
      0,
    )
    assert.equal(
      (
        await find('search_activity_details', [
          { name: 'Drums', detail: 'Bass' },
          { name: 'TLT', year: 2025, detail: 'Teaching' },
        ])
      ).length,
      1,
    )
    assert.equal(
      (await find('search_activity_details', [{ name: 'TLT', year: 2024, detail: 'Teaching' }]))
        .length,
      0,
    )
    for (const name of names) {
      assert.equal(
        (
          await find('search_event_details', [
            { name, year: 2024, detail: '1st Place' },
            { name, year: 2025, detail: '2nd Place' },
          ])
        ).length,
        1,
      )
      assert.equal(
        (await find('search_event_details', [{ name, year: 2024, detail: '2nd Place' }])).length,
        0,
      )
