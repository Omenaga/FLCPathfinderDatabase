...
test('member revision preserves history and enforces roles, ranges and access',async()=>{
 ...
 assert.deepEqual(afterAdd.pbe[0].history.find(r=>r.year==='2012-13').books,['1 Thessalonians','2 Thessalonians','Acts'])
 ...
 injected.red_zone_tents.push({pathfinder_id:pfId,year:'2011-12',placement:'Participation'})
 ...
 const freshId=(await create('10000000-0000-0000-0000-000000000090','New Editor','parent',null)).rows[0].id
 ...
 fresh.pbe.push({history:[{year:null,books:['Ignored'],results:{Area:'1st Place'}}]})
 ...
 await assert.rejects(db.query('delete from pathfinders where id=$1',[staffId]),e=>e.code==='42501')
