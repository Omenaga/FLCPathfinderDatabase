...
test('Edit Profile confirms once, updates history and returns to the refreshed profile',async({page})=>{
 ...
 await editor.getByRole('region',{name:'Burning Twine',exact:true}).getByLabel('Placement').selectOption('2nd Place')
 await editor.getByRole('button',{name:'Save Changes',exact:true}).click()
 expect(payload).toBeUndefined()
 await page.getByRole('dialog',{name:'Confirm Profile Changes'}).getByRole('button',{name:'Confirm',exact:true}).click()
 await expect(page.getByRole('dialog',{name:'Confirm Profile Changes'}).getByRole('button',{name:'Back to Edit',exact:true})).toBeDisabled()
 await expect.poll(()=>!!release).toBe(true)
...
test('Edit Profile receipt keeps failed edits and supports going back without saving',async({page})=>{
...
test('Edit Profile shows all classes, year dropdowns, automatic PBE books and new entries',async({page})=>{
 const {editor}=await setupEditor(page)
 ...
 await expect(explorer.getByLabel('Outcome').getByRole('option',{name:'N/A',exact:true})).toHaveCount(1)
 ...
 await expect(year.getByRole('option',{name:`${current}-${String(current+1).slice(-2)}`,exact:true})).toHaveCount(1)
 ...
 expect(payload.p_profile.pathfinders[0].levels.some((row:any)=>row.outcome==='N/A')).toBe(false)
...
test('removing instances stays in the draft and appears in the confirmation receipt',async({page})=>{
 ...
 await expect(levels.getByRole('region',{name:'Friend',exact:true}).getByLabel('Outcome')).toHaveValue('')
 await editor.getByRole('region',{name:'Drums',exact:true}).getByRole('button',{name:'Remove Drums Entry 1',exact:true}).click()
 await expect(editor.getByRole('region',{name:'Drums',exact:true}).getByLabel('Year')).toHaveCount(0)
 await editor.getByRole('region',{name:'Burning Twine',exact:true}).getByRole('button',{name:'Remove Burning Twine Entry 1',exact:true}).click()
 await editor.getByRole('region',{name:'Honors',exact:true}).getByRole('button',{name:'Remove Honors Entry 1',exact:true}).click()
 const archery=editor.getByRole('region',{name:'Archery',exact:true})
 await archery.getByRole('button',{name:'Add Archery',exact:true}).click()
 ...
 await expect(review.getByRole('region',{name:'Updated',exact:true})).toContainText('Reviewed note')
