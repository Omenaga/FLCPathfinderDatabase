...
- Current results: Profile icon, First Name, Last Name, Status, and Class/Titles.
- Status choices: Pathfinder, Staff, Parent, Not Active; no current data means Not Active.
- Search names, periods/calendar years, Pathfinder levels (including outcome-free Master Guide), historical Staff titles, activity teams/instruments/operations, and Red Zone placements. Selections within a category use OR; selected categories combine with AND.
...
Master Guide is an earned achievement rather than a Staff title; Master Guide Leader remains a title. Search offers a Pathfinder/Staff history selector and closes option menus after a selection. Current Activities has been removed from registration; historical activities remain unchanged. Apply the new migration before deploying this frontend. Migration backups retain retired registration values and affected Master Guide records privately.
