-- Detail records are authoritative; retain legacy core arrays without requiring them.
begin;
create or replace function private.check_participation() returns trigger
...
$$;
create or replace function private.check_member_participation() returns trigger
language plpgsql security definer set search_path = '' as $$
begin
 new.updated_at := now();
 return new;
