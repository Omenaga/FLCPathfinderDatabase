begin;
lock table public.current_data, public.staff_titles in access exclusive mode;
create table private.current_title_json_backup as select * from public.current_data;
revoke all on private.current_title_json_backup from public,anon,authenticated;
drop view public.member_search;
alter table public.current_data alter column current_title type jsonb
 using case when current_title is null then null else jsonb_build_array(current_title) end;
alter table public.current_data drop constraint current_data_current_activities_check;
create or replace function private.validate_current_title() returns trigger
language plpgsql security definer set search_path='' as $$
declare allowed text[];
begin
 if new.status='staff' then
  new.current_activities := '["N/A"]'::jsonb;
 elsif tg_op='UPDATE' then
  if old.status='staff' and new.current_activities='["N/A"]'::jsonb then new.current_activities := '[]'::jsonb; end if;
 end if;
...
update public.current_data set current_activities='["N/A"]' where status='staff';
alter table public.current_data add constraint current_data_current_activities_check check (
 (status='staff' and current_activities='["N/A"]'::jsonb) or
 (status<>'staff' and private.valid_string_array(current_activities,array['Drill','Drums','PBE','TLT']))
);
