...
select p.id,p.name,p.years_active,p.levels,p.created_at,p.updated_at,
  ...
  coalesce((select jsonb_agg(distinct jsonb_build_object('name',name,'year',year,'detail',detail)) from (
    ...
    cross join lateral unnest(array[split_part(y,'-',1),split_part(y,'-',2)]) calendar_year where d.pathfinder_id=p.id
    union all
    select 'TLT', (entry->>'year')::integer, operation
    from public.tlt t cross join lateral jsonb_array_elements(t.history) entry
    cross join lateral jsonb_array_elements_text(entry->'operations') operation where t.pathfinder_id=p.id
  ...
  coalesce((select jsonb_agg(distinct jsonb_build_object('name',name,'year',year,'detail',placement)) from (
    select 'Drill Performance'::text as name, year, placement from public.red_zone_drill_performance where pathfinder_id=p.id
    union all
    select 'Drum Performance'::text as name, year, placement from public.red_zone_drum_performance where pathfinder_id=p.id
    union all
    select 'Honor Evaluations'::text as name, year, placement from public.red_zone_honor_evaluations where pathfinder_id=p.id
    union all
    select 'Bible Events'::text as name, year, placement from public.red_zone_bible_events where pathfinder_id=p.id
    union all
    select 'Knots Relay'::text as name, year, placement from public.red_zone_knots where pathfinder_id=p.id
    union all
    select 'Tents'::text as name, year, placement from public.red_zone_tents where pathfinder_id=p.id
    union all
    select 'Jump Rope'::text as name, year, placement from public.red_zone_jump_rope where pathfinder_id=p.id
    union all
    select 'Archery'::text as name, year, placement from public.red_zone_archery where pathfinder_id=p.id
    union all
    select 'Lashing'::text as name, year, placement from public.red_zone_lashing where pathfinder_id=p.id
    union all
    select 'Burning Twine'::text as name, year, placement from public.red_zone_burning_twine where pathfinder_id=p.id
