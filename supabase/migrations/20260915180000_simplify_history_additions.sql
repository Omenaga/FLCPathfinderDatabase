...
drop function public.add_to_records(integer[],text,text,jsonb);

create function public.add_to_records(p_ids integer[], p_year text, p_entry jsonb)
returns jsonb language plpgsql security invoker set search_path='' as $$
declare
 record_role text; person integer; person_name text; person_status text; years jsonb; levels_value jsonb;
...
begin
 ...
 for person in select distinct unnest(p_ids) order by 1 loop
  ...
  begin
   ...
   record_role := case when kind='staff' then 'staff' when kind='level' then 'pathfinder' when person_status='staff' then 'staff' else 'pathfinder' end;
   ...
   if kind='level' then
    ...
    select id,d.years into row_id,old_history from public.drill d where pathfinder_id=person and history_role=record_role and team is not distinct from (p_entry->>'team') for update;
    changed := row_id is null or not (old_history @> jsonb_build_array(p_year));
    if row_id is null then insert into public.drill(pathfinder_id,history_role,team,years) values(person,record_role,p_entry->>'team',jsonb_build_array(p_year));
    ...
    if kind='staff' then
     ...
     execute format('select id,history from public.%I where pathfinder_id=$1 and history_role=$2 for update',table_name) into row_id,old_history using person,record_role;
    end if;
    ...
    if row_id is null then
     ...
     else execute format('insert into public.%I(pathfinder_id,history_role,history) values($1,$2,$3)',table_name) using person,record_role,merged; end if;
    ...
    select id into row_id from public.honors_earned where pathfinder_id=person and honor_id=(p_entry->>'honor_id')::integer and year_earned=p_year and history_role=record_role;
    changed := row_id is null;
    if changed then insert into public.honors_earned(pathfinder_id,honor_id,year_earned,history_role) values(person,(p_entry->>'honor_id')::integer,p_year,record_role); end if;
    ...
    if table_name in ('red_zone_honor_evaluations','red_zone_bible_events') then
     ...
     execute format('select id,placement from public.%I where pathfinder_id=$1 and history_role=$2 and year=$3 and name=$4 for update',table_name)
       into row_id,existing_placement using person,record_role,p_year,btrim(p_entry->>'name');
    else
     execute format('select id,placement from public.%I where pathfinder_id=$1 and history_role=$2 and year=$3 for update',table_name)
       into row_id,existing_placement using person,record_role,p_year;
    end if;
    ...
    if changed then
     if table_name in ('red_zone_honor_evaluations','red_zone_bible_events') then
      execute format('insert into public.%I(pathfinder_id,history_role,year,name,placement) values($1,$2,$3,$4,$5)',table_name) using person,record_role,p_year,btrim(p_entry->>'name'),p_entry->>'placement';
     else execute format('insert into public.%I(pathfinder_id,history_role,year,placement) values($1,$2,$3,$4)',table_name) using person,record_role,p_year,p_entry->>'placement'; end if;
    end if;
   end if;
