...
-- Region names are keys, ensuring only one placement per region in each year.
create function private.valid_pbe_results(value jsonb) returns boolean
language plpgsql immutable set search_path='' as $$
declare region text; placement jsonb;
begin
 if value is null or jsonb_typeof(value)<>'object' then return false; end if;
 for region,placement in select * from jsonb_each(value) loop
  if region not in ('Area','State','Union','Divisional') or jsonb_typeof(placement)<>'string'
   or placement #>> '{}' not in ('1st Place','2nd Place','3rd Place','Participation') then return false; end if;
 end loop;
 return true;
end $$;
revoke all on function private.valid_pbe_results(jsonb) from public,anon;
grant execute on function private.valid_pbe_results(jsonb) to authenticated,service_role;
...
-- Add results without changing existing placements or losing books and other years.
create function private.append_pbe_history(history jsonb, period text, books jsonb, results jsonb)
returns jsonb language plpgsql immutable set search_path='' as $$
declare previous jsonb; merged jsonb; region text; placement jsonb;
begin
...
grant execute on function private.append_pbe_history(jsonb,text,jsonb,jsonb) to authenticated;
